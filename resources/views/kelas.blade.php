...
<head>
  ...
  <title>Data Matkul - Sistem Akademik</title>
  ...
  <main class="flex-1 p-8 min-h-screen overflow-auto">
    <section>
      <h2 class="text-3xl font-bold text-gray-700 mb-6 flex items-center gap-2">
        <i data-feather="users" class="text-blue-600"></i> Data Matkul
      </h2>
          ...
          <tbody id="kelasTableBody">
            @foreach ($kelas as $index => $kls)
              <tr class="hover:bg-gray-50">
                ...
                <td class="py-2 px-4 border border-gray-300">{{ $kls['kode_matkul'] }}</td>
                <td class="py-2 px-4 border border-gray-300">{{ $kls['nama_matkul'] }}</td>
                <td class="py-2 px-4 border border-gray-300">{{ $kls['semester'] }}</td>
                <td class="py-2 px-4 border border-gray-300">{{ $kls['sks'] }}</td>
                <td class="py-2 px-4 border border-gray-300 text-center">
                  <a href="{{ route('kelas.edit', $kls['kode_matkul']) }}" class="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600">Edit</a>
                  <form action="{{ route('kelas.destroy', $kls['kode_matkul']) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                    @csrf
                    @method('DELETE')
