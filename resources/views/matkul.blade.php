...
<body class="bg-gray-100 font-sans min-h-screen flex">
  ...
  <aside class="w-64 bg-blue-800 min-h-screen text-white p-5 flex flex-col">
    ...
    <nav class="space-y-3">
      ...
      <a href="{{ route('matkul.index') }}" class="flex items-center gap-3 px-3 py-2 rounded bg-white text-blue-800 font-semibold">
  ...
  <main class="flex-1 p-8 min-h-screen overflow-auto">
    <section>
      <h2 class="text-3xl font-bold text-gray-700 mb-6 flex items-center gap-2">
        <i data-feather="book-open" class="text-blue-600"></i> Data Mata Kuliah
      </h2>

      <!-- Tombol Tambah & Search -->
      <div class="flex justify-between items-center mb-4">
        <a href="{{ route('matkul.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
          + Tambah Data
        </a>
        <form method="GET" action="{{ route('matkul.index') }}" class="relative w-64">
          <input 
            id="searchInput" 
            type="text" 
            name="search"
        ...
        <table class="min-w-full table-auto border border-gray-300">
          <thead>
            <tr class="bg-blue-100 text-left text-gray-700 text-sm uppercase">
              ...
              <th class="py-3 px-4 border border-gray-300">Kode Matkul</th>
              <th class="py-3 px-4 border border-gray-300">Nama Matkul</th>
              <th class="py-3 px-4 border border-gray-300">Semester</th>
              <th class="py-3 px-4 border border-gray-300">SKS</th>
          ...
          <tbody id="matkulTableBody">
            @foreach ($matkul as $index => $mk)
              <tr class="hover:bg-gray-50">
                ...
                <td class="py-2 px-4 border border-gray-300">{{ $mk['kode_matkul'] }}</td>
                <td class="py-2 px-4 border border-gray-300">{{ $mk['nama_matkul'] }}</td>
                <td class="py-2 px-4 border border-gray-300">{{ $mk['semester'] }}</td>
                <td class="py-2 px-4 border border-gray-300">{{ $mk['sks'] }}</td>
                <td class="py-2 px-4 border border-gray-300 text-center">
                  <a href="{{ route('matkul.edit', $mk['kode_matkul']) }}" class="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600">Edit</a>
                  <form action="{{ route('matkul.destroy', $mk['kode_matkul']) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                    @csrf
                    @method('DELETE')
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function () {
      ...
      const tableBody = document.getElementById("matkulTableBody");
